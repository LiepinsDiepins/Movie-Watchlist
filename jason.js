...
const watchmodeApiKey = 'YUPKihGylNlCVQG4uqKDMYyfLwuwk6lCtXgoxf7z';
const randomTitles= [
    'Inception', 'The Matrix', 'Forrest Gump', 'Interstellar',
      'The Lion King', 'Pulp Fiction', 'The Godfather', 'Parasite'
];

async function loadRandomMovie() {
    const randomTitle = randomTitles[Math.floor(Math.random() * randomTitles.length)];
    document.getElementById('movieTitle').textContent = randomTitle;

    try {
        const searchRes = await fetch(`https://api.watchmode.com/v1/search/?apiKey=${watchmodeApiKey}&search_field=name&search_value=${encodeURIComponent(randomTitle)}`);
        const searchData = await searchRes.json();
        ...
        if (searchData.title_results.length > 0) {
          ...
          const watchLinkDiv = document.getElementById('watchLink');
          if (bestSource) {
            watchLinkDiv.innerHTML = `<p><strong>Skaties šeit:</strong> ${bestSource.web_url}${bestSource.name}</a></p>`;
          } else {
            watchLinkDiv.innerHTML = `<p><strong>Nav pieejama skatīšanās saite.</strong></p>`;
          }
        } else {
          document.getElementById('watchLink').innerHTML = `<p><strong>Filma nav atrasta Watchmode datubāzē.</strong></p>`;
        }
      } catch (error) {
        console.error('Kļūda:', error);
        document.getElementById('watchLink').innerHTML = `<p><strong>Radās kļūda, mēģini vēlreiz.</strong></p>`;
      }
    }
